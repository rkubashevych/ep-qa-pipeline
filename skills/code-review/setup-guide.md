# Setup guide for the Code Review skill

This skill checks the test cases against the PR code. The review
logic (classification, what to look for, the process) is universal.
The only thing to configure is how the PR and the code are accessed.
...
### 2. How does the AI access the PR code?
...
- Branch mode (git CLI: `git fetch`/`diff`/`show`, plus the repository
  `src` endpoint) needs only `read:repository:bitbucket` — and the
  branch name equals the issue key (e.g. `EP-54610`). This works with
  the existing `bitbucket-git-cli` token.
...
### Step 3: SKILL.md — "Recommended flow" section

The actual commands for Bitbucket Cloud are set in the

...
> (fetch/diff/show + the repository `src` endpoint) works fully. Prefer
> branch mode: the branch name is the issue key (e.g. `EP-54610`). Only
> use PR-URL / REST mode if the token has the `read:pullrequest:bitbucket`
> scope.

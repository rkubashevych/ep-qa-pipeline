...
> **Stage 10 = `qa-manual-results`.** The return leg: when the tester hands back the filled run sheet (or a TC/Result/Notes table), this stage joins the results **by TC id, never row position**, reconciles them against the machine verdicts on the pass's **QA Service test run**, records the human verdicts into the same run (`source: manual`; a re-record supersedes and keeps history — that is the **retraction**), closes the run, and posts the first human-facing summary. A retraction of a verdict that reached a Jira comment is also posted on the ticket where it was published. Until stage 10 runs, the machine verdicts are provisional and the run stays open. See `skills/qa-pipeline/references/test-runs.md`.
